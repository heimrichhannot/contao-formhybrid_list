...
class ModuleList extends \Module
{
    ...
    protected function generateFields($objItem)
    {
        ...
        if ($this->isTableList) {
            ...
            $fields = $this->limitFormattedFields ? deserialize($this->formattedFields, true) : (isset($arrDca['fields']) && \is_array($arrDca['fields']) ? array_keys($arrDca['fields']) : []);

            foreach ($arrDca['fields'] as $strField => $arrData) {

                if (!in_array($strField, $fields)) {
                    $arrItem['fields'][$strField] = FormHelper::escapeAllEntities($this->formHybridDataContainer, $strField, $objItem->{$strField});
                    continue;
                }
    ...
    protected function applyDefaultFilters()
    {
        foreach ($this->arrDefaultValues as $arrDefaultValue) {
            ...
            if ($this->objFilterContext->addProximitySearch && $this->fieldInDefaultValues(FormHybridList::PROXIMITY_SEARCH_RADIUS)) {
                switch ($strField) {
    ...
    protected function fieldInDefaultValues($field)
    {
        $defaultValues = deserialize($this->formHybridDefaultValues, true);

        foreach ($defaultValues as $defaultValue) {
            if (false !== array_search($field, $defaultValue)) {
                return true;
            }
        }
